...
const AllocateTask = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const url = "http://localhost:8086/api/tasks";
            const { data: res } = await axios.post(url, data);
            setSuccessMessage(res.message); // Set success message
            alert(res.message);
            sendStatusEmail( data.taskName,  data.taskDescription, data.validity)
    ...
    const sendStatusEmail = ( taskname,  description , validity) => {
        const emailParams = {
          subject:'Task Notification',
          task_name: taskname,
          description: description,
          validity: validity,
          from_name:'leilamarouaalouache@gmail.com'
        };
    
        emailjs
          .send('service_Versa', 'template_y9hay6m', emailParams, 'E-A2ibcC_EBHU7QXS')
          .then(
            () => {
