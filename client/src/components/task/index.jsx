...
const TasksList = ({ email }) => {
    ...
    useEffect(() => {
        const fetchTasks = async () => {
            try {
                const response = await axios.get("http://localhost:8086/api/tasks", {
                    params: {
                        email: email
                    }
                });
    ...
    const handleUpdateStatus = async (taskId, newStatus) => {
        try {
            ...
            await axios.put(`http://localhost:8086/api/tasks/${taskId}`, { status: newStatus });
        } catch (error) {
            ...
            <ul className={styles.task_list}>
                {sortedTasks.map(task => (
                    <li key={task._id} className={styles.task_item}>
                        <h3>{task.taskName}</h3>
                        <p> <span>Task Description :</span> {task.taskDescription}</p>
                        <div className={styles.details}>
                            {/* <p><span>Allocated To:</span> {task.allocatedTo}</p> */}
                            <p><span>Status:</span> {task.status}</p>
                            <p><span>Created At:</span> {new Date(task.createdAt).toLocaleDateString()}</p>
                            <p><span>Validity:</span> {new Date(task.validity).toLocaleDateString()}</p>
                        </div>
